...
Dean Karlan at Yale and John List at the University of Chicago conducted a large-scale natural field experiment to test the impact of matching donations on charitable giving. In collaboration with a liberal nonprofit organization in the United States, they sent out over 50,000 direct mail solicitations to previous donors, randomly assigning individuals to receive either a standard fundraising appeal (control) or a version with a matching grant (treatment). 

The treatment group was further randomized across different match ratios (1:1, 2:1, 3:1), different matching cap amounts ($25k, $50k, $100k, or unspecified), and varying suggested donation amounts. This design allowed the authors to isolate the causal effects of matching offers and their specific structures on donation behavior.

Their key finding: while the presence of any matching grant significantly increased both the response rate and revenue per solicitation, increasing the match ratio beyond 1:1 (to 2:1 or 3:1) did not lead to additional gains — a surprising result that challenges conventional fundraising wisdom.

This project replicates their findings using the provided dataset and Stata code, and presents the results in a blog-style format.

## Data

### Description

The dataset consists of 50,083 prior donors, with randomized assignment to control and treatment groups. Key variables include:
- `treatment`: indicator for whether the individual received any matching offer
- `gave`: whether the individual donated
- `amount`: how much the individual donated
- `ratio`: the match ratio offered (1:1, 2:1, or 3:1)
- `size`: maximum amount the leadership donor was willing to match
- `ask`: suggested donation amount

Below we explore basic outcome differences by treatment group.
...
```{r}
#| fig-cap: "Response Rate by Treatment Group"
#| echo: false
grouped <- data.frame(
  Group = c("Control", "Treatment"),
  ResponseRate = c(0.0179, 0.0220)
)
barplot(grouped$ResponseRate, names.arg = grouped$Group, col = "skyblue",
        ylab = "Donation Response Rate", main = "Response Rate by Treatment Group")
```

We observe a clear positive impact of offering a matching donation: the treatment group had a response rate of **2.20%** compared to **1.79%** in the control group.

### Effect by Match Ratio

```{r}
#| fig-cap: "Response Rate by Match Ratio"
#| echo: false
ratios <- c("1:1", "2:1", "3:1")
rates <- c(nan, 0.0207, 0.0226)
barplot(rates, names.arg = ratios, col = "coral",
        ylab = "Response Rate", main = "Response Rate by Match Ratio")
```

Interestingly, the response rate remains fairly constant across different match ratios, suggesting that **increasing the match beyond 1:1 did not meaningfully improve donation rates**. This confirms Karlan & List's conclusion that larger match ratios don’t always yield higher returns.
